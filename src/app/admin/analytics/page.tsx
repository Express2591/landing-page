      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        <div className="border p-4 rounded">
          ...
          <p className="text-2xl">
            {analytics.sends[new Date().toISOString().split('T')[0]]?.total || 0}
          </p>
        </div>
        <div className="border p-4 rounded">
          <h3 className="font-bold">Opens Today</h3>
          <p className="text-2xl">
            {Object.values(analytics.opens[new Date().toISOString().split('T')[0]] || {}).reduce((a, b) => a + b, 0)}
          </p>
        </div>
        <div className="border p-4 rounded">
          <h3 className="font-bold">Clicks Today</h3>
          <p className="text-2xl">
            {Object.values(analytics.clicks[new Date().toISOString().split('T')[0]] || {}).reduce((a, b) => a + b, 0)}
          </p>
        </div>
      </div>
